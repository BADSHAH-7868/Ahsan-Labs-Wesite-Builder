...
const FREE_MODELS = [
  ...
  // { id: 'nvidia/nemotron-nano-9b-v2:free', name: 'NemoTron', provider: 'Nvidia'},
    ...
    >
      <div className="bg-gray-900/50 backdrop-blur-xl rounded-xl border border-purple-900/50 p-8 shadow-lg hover:shadow-purple-500/30">
        <div className="text-center mb-8">
          ...
          <h3 className="text-3xl font-bold mb-2 bg-gradient-to-r from-purple-400 to-cyan-400 bg-clip-text text-transparent">
            Choose Your AI Model
          </h3>
          <p className="text-gray-300">Select a free model from OpenRouter to power your automation script generation</p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="relative">
              <motion.button
                className="w-full bg-gray-900/50 border border-purple-900/50 rounded-xl px-4 py-4 text-left flex items-center justify-between hover:border-purple-500/50 transition-all duration-300"
                  ...
                  {selectedModelData ? (
                    <>
                      ...
                      <div>
                        ...
                        <div className="text-sm text-gray-300">{selectedModelData.provider}</div>
                      </div>
                    </>
                  ) : (
                    <span className="text-gray-300">Select a model...</span>
                  )}
                </div>
              ...
              {isDropdownOpen && (
                <motion.div 
                  className="absolute top-full left-0 right-0 mt-2 bg-gray-900/90 backdrop-blur-xl border border-purple-900/50 rounded-xl overflow-hidden z-50"
                ...
                >
                  <div className="p-3 border-b border-purple-900/50">
                    <input
                      ...
                      className="w-full bg-gray-900/50 border border-purple-900/50 rounded-lg px-3 py-2 text-white placeholder-gray-300 focus:outline-none focus:border-purple-500"
                      value={searchTerm}
                  ...
                  <div className="max-h-64 overflow-y-auto">
                    {filteredModels.map((model) => (
                      <motion.button
                        key={model.id}
                        className="w-full px-4 py-3 text-left hover:bg-gray-800/50 transition-all duration-200 flex items-center justify-between group"
                        onClick={() => {
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-3">
              <Key className="inline w-4 h-4 mr-2 text-purple-400" />
              OpenRouter API Key
            </label>
            <motion.input
              type="password"
              placeholder="sk-or-..."
              className="w-full bg-gray-900/50 border border-purple-900/50 rounded-xl px-4 py-4 text-white placeholder-gray-300 focus:outline-none focus:border-purple-500 transition-all duration-300"
              value={apiKey}
              onChange={(e) => setApiKey(e.target.value)}
              whileFocus={{ scale: 1.02 }}
            />
            <p className="text-xs text-gray-300 mt-2">
              Get your free API key from <a href="https://openrouter.ai/models?max_price=0" target="_blank" rel="noopener noreferrer" className="text-purple-400 hover:underline">OpenRouter.ai</a>
            </p>
          </div>
        </div>
        ...
        <div className="mt-8 flex justify-end">
          <motion.button
            className={`px-8 py-4 rounded-xl font-medium transition-all duration-300 flex items-center space-x-2 ${
              isFormValid
                ? 'bg-gradient-to-r from-purple-600 to-blue-600 text-white hover:shadow-purple-500/50 shadow-lg'
